...
-- Use "local" header lines & Remove unused locals.
--
local action_id_touch       = hash("touch")
local action_id_text        = hash("text")
local action_id_marked_text = hash("marked_text")
local action_id_backspace   = hash("backspace")

local active_input_marked = ""
local active_node = nil
local input_nodes = {}
local color_base = vmath.vector4(1,1,1,1)
local color_active = vmath.vector4(1,1,1,1)
local color_enabled = vmath.vector4(1,1,1,1)
local color_disabled = vmath.vector4(1,1,1,0.2)
...
function dirtylarry.input(self, node, action_id, action, type, empty_text)
	...
	if (not input_nodes[key]) then
		input_nodes[key] = { id = node, data = gui.get_text(node_content), active = false }
	end

	local input_node = input_nodes[key]
	...
	if (not active_node or active_node ~= input_node) and input_node.active then
		...
		gui.animate(active_node_bg, "color", color_base, gui.EASING_OUTCUBIC, 0.2)
	...
	if hit_test(self, node_bg, action_id, action) then
		...
		gui.animate(node_bg, "color", color_active, gui.EASING_OUTCUBIC, 0.2)
	...
	--
	elseif (action_id == action_id_touch) and (action.pressed) then
		active_node = nil
		-- input_node.data = ""
	----------------
	...
	if (active_node == input_node) then

		-- new raw text input
		if (action_id == action_id_text) then
			active_node.data = active_node.data .. action.text
			active_input_marked = ""

		-- new marked text input (uncommitted text)
		elseif (action_id == action_id_marked_text) then
			active_input_marked = action.text

		-- input deletion
		elseif (action_id == action_id_backspace and (action.pressed or action.repeated)) then
			local last_s = 0
			for uchar in string.gmatch(active_node.data, utf8_gfind) do
			  last_s = string.len(uchar)
			end

			active_node.data = string.sub(active_node.data, 1, string.len(active_node.data) - last_s)
		end

		-- set text color
		gui.set_color(node_content, color_enabled)

		-- if current input is active, include marked text in output
		text_output = active_node.data .. active_input_marked

		-- get text metrics for both raw input data and marked text
		local m_t = gui.get_text_metrics(gui.get_font(node_content), active_node.data, 0, false, 0, 0)
		local m_m = gui.get_text_metrics(gui.get_font(node_content), active_input_marked, 0, false, 0, 0)

		-- set cursor (and marked text bg)
		gui.set_position(node_cursor, vmath.vector3(4 + m_t.width, 0, 0))
		if (active_input_marked and #active_input_marked) then
	...
	if (type == gui.KEYBOARD_TYPE_PASSWORD) then
		local masked_text = ""
		for uchar in string.gmatch(text_output, utf8_gfind) do
	...
	if (empty_text and string.len(text_output) == 0) then
		gui.set_color(node_content, color_disabled)
...
function dirtylarry.slider(self, node_str, action_id, action, min_value, max_value, value)
	...
	if action.released and action_id == action_id_touch and active_node == node_sa then
		active_node = nil
	elseif hit and action.pressed and active_node == nil then
		active_node = node_sa
	end
	...
	gui.set_text(node_value, string.format(slider_fmt, slider_value))
