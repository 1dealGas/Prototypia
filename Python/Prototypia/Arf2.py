...
class PnOverlapProhibition		(E):pass

def NonManual(f):
	def decorated(*args, **kwargs) -> None:
		if len( inspect.stack() ) < 3 : raise ManualProhibition('''The "new" method is prohibited to call manually.''')
...
def PosNodeSorter(a,b) -> int:
	...
	else: raise PnOverlapProhibition('''Adding multiple PosNodes with the same Bartime to a single WishGroup is prohibited.''')
...
class Arf2Prototype:
	...
	_bpms:list				= []	# (Bartime, BPM)
	_scales_layer1:list		= []	# (Bartime, Scale)
	_scales_layer2:list		= []	# Bartime>=0   BPM>0
